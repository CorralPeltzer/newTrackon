...
<div class="container">
    ...
    <p>newTrackon is a service to monitor the status and health of existing open and public trackers that anyone can
        use. A meta-tracker if you will. You can add any of the tracker announce URLs listed here to any of your
        torrents, or submit any other open/public trackers you might know of.</p>
    <p><b>To get a client-ready list of all trackers with more than 95% of uptime, go to the <a href="/list">List</a>
        section.</b></p>
...
% endif
    ...
    <td>${"%.2f" % t.uptime}%</td>
% if t.status == 1:
    <td data-sort="${t.status_epoch}" class="up"><b>${t.status_readable}</b></td>
<% lt += 1 %>
% else:
    <td data-sort="${t.status_epoch}" class="down"><b>${t.status_readable}</b></td>
<% dt += 1 %>
% endif
...
% endfor
% endif
    <caption style="text-align: right;"><b>Live trackers</b>: ${lt} / <b>Trackers down</b>: ${dt} / <b>Total
        trackers</b>: ${len(trackers)}</caption>
</table>
    <noscript>
        <style>#trackon_table {
            visibility: visible
    ...
    <p style="text-align: right;">* Based on the last 1000 checks. The time depends on the update interval set by the
        tracker, and can vary from 6 to 40 days.</p>
</div>
