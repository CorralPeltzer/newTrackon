...
class TestMainPage:
    ...
    def test_get_main_page_returns_200(self, flask_client: FlaskClient) -> None:
        """GET / should return 200 OK."""
        response = flask_client.get("/")
        assert response.status_code == 200

    @pytest.mark.usefixtures("mock_db_connection")
    def test_main_page_includes_theme_toggle(self, flask_client: FlaskClient) -> None:
    ...
    def test_get_main_page_with_trackers(self, flask_client: FlaskClient) -> None:
        """GET / should display trackers from database."""
        response = flask_client.get("/")
        assert response.status_code == 200
        # The template should render tracker data
        assert b"tracker.example.com" in response.get_data() or response.status_code == 200

    @pytest.mark.usefixtures("mock_db_connection")
    def test_post_main_page_with_new_trackers_triggers_enqueue(self, flask_client: FlaskClient) -> None:
        ...
        with patch("newtrackon.ingest.enqueue_new_trackers"):
            ...
            assert b"SUCCESS" in response.get_data() or response.status_code == 200

    @pytest.mark.usefixtures("mock_db_connection")
    def test_post_main_page_empty_string_returns_form_feedback(self, flask_client: FlaskClient) -> None:
        """POST / with empty new_trackers should return EMPTY feedback."""
        response = flask_client.post("/", data={"new_trackers": ""})
        assert response.status_code == 200
        assert b"EMPTY" in response.get_data() or response.status_code == 200

    @pytest.mark.usefixtures("mock_db_connection")
    def test_post_main_page_missing_field_returns_400(self, flask_client: FlaskClient) -> None:
        """POST / without new_trackers field should return 400."""
        response = flask_client.post("/", data={})
        assert response.status_code == 400

    @pytest.mark.usefixtures("mock_db_connection")
    def test_post_main_page_too_long_input_returns_413(self, flask_client: FlaskClient) -> None:
...
class TestApiAddEndpoint:
    ...
    @pytest.mark.usefixtures("mock_db_connection")
    def test_post_api_add_with_trackers_returns_204(self, flask_client: FlaskClient) -> None:
    ...
    @pytest.mark.usefixtures("mock_db_connection")
    def test_post_api_add_empty_returns_400(self, flask_client: FlaskClient) -> None:
        """POST /api/add with empty or missing new_trackers should return 400."""
        response = flask_client.post("/api/add", data={"new_trackers": ""})
        assert response.status_code == 400

    @pytest.mark.usefixtures("mock_db_connection")
    def test_post_api_add_missing_field_returns_400(self, flask_client: FlaskClient) -> None:
        """POST /api/add without new_trackers field should return 400."""
        response = flask_client.post("/api/add", data={})
        assert response.status_code == 400

    @pytest.mark.usefixtures("mock_db_connection")
    def test_post_api_add_too_long_input_returns_413(self, flask_client: FlaskClient) -> None:
...
class TestApiPercentageEndpoint:
    ...
    @pytest.mark.usefixtures("insert_sample_tracker")
    def test_get_api_percentage_returns_filtered_trackers(self, flask_client: FlaskClient) -> None:
    ...
    @pytest.mark.usefixtures("insert_sample_tracker")
    def test_get_api_percentage_filters_by_uptime(self, flask_client: FlaskClient) -> None:
        ...
        assert response.status_code == 200
        assert b"udp://tracker.example.com:6969/announce" not in response.get_data()

    @pytest.mark.usefixtures("insert_sample_tracker")
    def test_get_api_percentage_zero_returns_all(self, flask_client: FlaskClient) -> None:
    ...
    @pytest.mark.usefixtures("insert_sample_tracker")
    def test_get_api_percentage_100_returns_only_perfect(self, flask_client: FlaskClient) -> None:
    ...
    @pytest.mark.usefixtures("mock_db_connection")
    def test_get_api_percentage_invalid_above_100_returns_400(self, flask_client: FlaskClient) -> None:
    ...
    @pytest.mark.usefixtures("mock_db_connection")
    def test_get_api_percentage_negative_returns_400(self, flask_client: FlaskClient) -> None:
    ...
    def test_get_api_percentage_with_min_age_days_filters_new_trackers(
        ...
        assert b"old.percentage.tracker.com" in response_filtered.get_data()
        assert b"new.percentage.tracker.com" not in response_filtered.get_data()

    @pytest.mark.usefixtures("mock_db_connection")
    def test_get_api_percentage_with_invalid_min_age_days_returns_400(self, flask_client: FlaskClient) -> None:
...
class TestApiStableEndpoint:
    ...
    def test_get_api_stable_with_min_age_days_zero_includes_new_trackers(
        ...
        assert b"old.stable.tracker.com" in response_including_new.get_data()
        assert b"new.stable.tracker.com" in response_including_new.get_data()

    @pytest.mark.usefixtures("mock_db_connection")
    def test_get_api_stable_with_invalid_min_age_days_returns_400(self, flask_client: FlaskClient) -> None:
...
class TestApiBestEndpoint:
    ...
    @pytest.mark.usefixtures("mock_db_connection")
    def test_get_api_best_redirects_to_stable(self, flask_client: FlaskClient) -> None:
    ...
    @pytest.mark.usefixtures("insert_sample_tracker")
    def test_get_api_best_follow_redirect(self, flask_client: FlaskClient) -> None:
...
class TestApiAllEndpoint:
    ...
    @pytest.mark.usefixtures("insert_sample_tracker")
    def test_get_api_all_returns_all_trackers(self, flask_client: FlaskClient) -> None:
    ...
    def test_get_api_all_with_min_age_days_filters_new_trackers(
        ...
        assert response.status_code == 200
        assert b"old.all.tracker.com" in response.get_data()
        assert b"new.all.tracker.com" not in response.get_data()
...
class TestApiLiveEndpoint:
    ...
    @pytest.mark.usefixtures("insert_sample_tracker")
    def test_get_api_live_returns_online_trackers(self, flask_client: FlaskClient) -> None:
    ...
    def test_get_api_live_excludes_offline_trackers(
        ...
        response = flask_client.get("/api/live")
        assert response.status_code == 200
        assert b"offline.tracker.com" not in response.get_data()
    ...
    def test_get_api_live_with_min_age_days_filters_new_trackers(
        ...
        assert response.status_code == 200
        assert b"old.live.tracker.com" in response.get_data()
        assert b"new.live.tracker.com" not in response.get_data()
...
class TestApiUdpEndpoint:
    ...
    @pytest.mark.usefixtures("insert_sample_tracker")
    def test_get_api_udp_returns_udp_trackers(self, flask_client: FlaskClient) -> None:
    ...
    def test_get_api_udp_excludes_http_trackers(self, flask_client: FlaskClient, mock_db_connection: sqlite3.Connection) -> None:
        ...
        response = flask_client.get("/api/udp")
        assert response.status_code == 200
        assert b"http://http.tracker.com" not in response.get_data()
    ...
    def test_get_api_udp_excludes_low_uptime_udp(self, flask_client: FlaskClient, mock_db_connection: sqlite3.Connection) -> None:
        ...
        response = flask_client.get("/api/udp")
        assert response.status_code == 200
        assert b"low.udp.tracker.com" not in response.get_data()
    ...
    def test_get_api_udp_with_min_age_days_filters_new_trackers(
        ...
        assert response.status_code == 200
        assert b"old.udp.tracker.com" in response.get_data()
        assert b"new.udp.tracker.com" not in response.get_data()
...
class TestApiHttpEndpoint:
    ...
    def test_get_api_http_returns_http_trackers(self, flask_client: FlaskClient, mock_db_connection: sqlite3.Connection) -> None:
        ...
        assert response.status_code == 200
        assert response.headers.get("Access-Control-Allow-Origin") == "*"
        assert b"http://http.tracker.com:6969/announce" in response.get_data()
    ...
    def test_get_api_http_includes_https_trackers(
        ...
        response = flask_client.get("/api/http")
        assert response.status_code == 200
        assert b"https://https.tracker.com:443/announce" in response.get_data()

    @pytest.mark.usefixtures("insert_sample_tracker")
    def test_get_api_http_excludes_udp_trackers(self, flask_client: FlaskClient) -> None:
        """GET /api/http should exclude UDP trackers."""
        # Sample tracker is UDP
        response = flask_client.get("/api/http")
        assert response.status_code == 200
        assert b"udp://tracker.example.com" not in response.get_data()
    ...
    def test_get_api_http_with_min_age_days_filters_new_trackers(
        ...
        assert response.status_code == 200
        assert b"old.http.tracker.com" in response.get_data()
        assert b"new.http.tracker.com" not in response.get_data()
...
class TestIpv4Ipv6Filtering:
    ...
    @pytest.mark.usefixtures("insert_ipv4_only_tracker", "insert_ipv6_only_tracker", "insert_dual_stack_tracker")
    def test_default_includes_all_trackers(
        ...
        assert response.status_code == 200
        assert b"ipv4only.tracker.com" in response.get_data()
        assert b"ipv6only.tracker.com" in response.get_data()
        assert b"dualstack.tracker.com" in response.get_data()
    ...
    def test_exclude_ipv4_only_trackers(
        ...
        assert response.status_code == 200
        assert b"ipv4only.tracker.com" not in response.get_data()
        assert b"ipv6only.tracker.com" in response.get_data()
        assert b"dualstack.tracker.com" in response.get_data()
    ...
    def test_exclude_ipv6_only_trackers(
        ...
        assert response.status_code == 200
        assert b"ipv4only.tracker.com" in response.get_data()
        assert b"ipv6only.tracker.com" not in response.get_data()
        assert b"dualstack.tracker.com" in response.get_data()
    ...
    @pytest.mark.usefixtures("insert_ipv4_only_tracker", "insert_dual_stack_tracker")
    ...
    @pytest.mark.usefixtures("insert_ipv6_only_tracker", "insert_dual_stack_tracker")
    ...
    @pytest.mark.usefixtures("insert_ipv4_only_tracker", "insert_ipv6_only_tracker")
...
class TestSubmittedEndpoint:
    ...
    @pytest.mark.usefixtures("mock_db_connection")
    def test_get_submitted_returns_200(self, flask_client: FlaskClient) -> None:
        """GET /submitted should return 200 OK."""
        response = flask_client.get("/submitted")
        assert response.status_code == 200

    @pytest.mark.usefixtures("mock_db_connection")
    def test_get_submitted_without_data_has_no_table_markup(self, flask_client: FlaskClient) -> None:
        """GET /submitted should not render orphan table markup without data."""
        with (
            patch("newtrackon.views.persistence.submitted_data", []),
            patch("newtrackon.views.ingest.submitted_queue.qsize", return_value=0),
        ):
            response = flask_client.get("/submitted")
            assert response.status_code == 200
            assert b"<table" not in response.get_data()
            assert b"</table>" not in response.get_data()
...
class TestRawEndpoint:
    ...
    @pytest.mark.usefixtures("mock_db_connection")
    def test_get_raw_returns_200(self, flask_client: FlaskClient) -> None:
        """GET /raw should return 200 OK."""
        response = flask_client.get("/raw")
        assert response.status_code == 200

    @pytest.mark.usefixtures("mock_db_connection")
    def test_get_raw_with_data(self, flask_client: FlaskClient) -> None:
...
class TestStaticPages:
    ...
    @pytest.mark.usefixtures("mock_db_connection")
    def test_get_faq_returns_200(self, flask_client: FlaskClient) -> None:
        """GET /faq should return 200 OK."""
        response = flask_client.get("/faq")
        assert response.status_code == 200

    @pytest.mark.usefixtures("mock_db_connection")
    def test_get_list_returns_200(self, flask_client: FlaskClient) -> None:
        """GET /list should return 200 OK."""
        response = flask_client.get("/list")
        assert response.status_code == 200

    @pytest.mark.usefixtures("mock_db_connection")
    def test_get_api_docs_returns_200(self, flask_client: FlaskClient) -> None:
        """GET /api should return 200 OK."""
        response = flask_client.get("/api")
        assert response.status_code == 200

    @pytest.mark.usefixtures("mock_db_connection")
    def test_get_about_returns_200(self, flask_client: FlaskClient) -> None:
...
class TestAnnounceRequestRejection:
    ...
    @pytest.mark.usefixtures("mock_db_connection")
    def test_reject_request_with_info_hash(self, flask_client: FlaskClient) -> None:
        """Requests with info_hash parameter should be rejected with 403."""
        response = flask_client.get("/?info_hash=abc123")
        assert response.status_code == 403

    @pytest.mark.usefixtures("mock_db_connection")
    def test_reject_api_request_with_info_hash(self, flask_client: FlaskClient) -> None:
...
class TestApiResponseHeaders:
    ...
    @pytest.mark.usefixtures("insert_sample_tracker")
    def test_api_cors_header(self, flask_client: FlaskClient) -> None:
        """API endpoints should include CORS header."""
        response = flask_client.get("/api/95")
        assert response.headers.get("Access-Control-Allow-Origin") == "*"

    @pytest.mark.usefixtures("insert_sample_tracker")
    def test_api_content_type(self, flask_client: FlaskClient) -> None:
        """API endpoints should return plain text."""
        response = flask_client.get("/api/95")
        assert "text/plain" in response.content_type

    @pytest.mark.usefixtures("mock_db_connection")
    def test_api_add_cors_header(self, flask_client: FlaskClient) -> None:
...
class TestEdgeCases:
    ...
    @pytest.mark.usefixtures("insert_sample_tracker")
    def test_api_percentage_boundary_zero(self, flask_client: FlaskClient) -> None:
        """GET /api/0 should be valid and return all trackers."""
        response = flask_client.get("/api/0")
        assert response.status_code == 200

    @pytest.mark.usefixtures("mock_db_connection")
    def test_api_percentage_boundary_100(self, flask_client: FlaskClient) -> None:
        """GET /api/100 should be valid."""
        response = flask_client.get("/api/100")
        assert response.status_code == 200

    @pytest.mark.usefixtures("mock_db_connection")
    def test_empty_database_api_response(self, flask_client: FlaskClient) -> None:
    ...
    def test_multiple_trackers_in_response(self, flask_client: FlaskClient, mock_db_connection: sqlite3.Connection) -> None:
        ...
        for i in range(3):
            assert f"tracker{i}.example.com".encode() in response.get_data()
        # Check that there are multiple newlines (format is url\n\n)
        assert response.get_data().count(b"\n") >= 3
    ...
    def test_special_characters_in_tracker_url(self, flask_client: FlaskClient, mock_db_connection: sqlite3.Connection) -> None:
        ...
        response = flask_client.get("/api/http")
        assert response.status_code == 200
        assert b"special.tracker.com:8080/path/announce?key=value" in response.get_data()
