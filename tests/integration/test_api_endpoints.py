...
class TestMainPage:
    ...
    def test_get_main_page_returns_200(self, flask_client, mock_db_connection):
        """GET / should return 200 OK."""
        response = flask_client.get("/")
        assert response.status_code == 200

    def test_get_main_page_with_trackers(self, flask_client, insert_sample_tracker):
    ...
    def test_post_main_page_with_new_trackers_triggers_enqueue(self, flask_client, mock_db_connection):
    ...
    def test_post_main_page_empty_string_returns_form_feedback(self, flask_client, mock_db_connection):
    ...
    def test_post_main_page_missing_field_returns_400(self, flask_client, mock_db_connection):
        """POST / without new_trackers field should return 400."""
        response = flask_client.post("/", data={})
        assert response.status_code == 400

    def test_post_main_page_too_long_input_returns_413(self, flask_client, mock_db_connection):
...
class TestApiAddEndpoint:
    ...
    def test_post_api_add_with_trackers_returns_204(self, flask_client, mock_db_connection):
    ...
    def test_post_api_add_empty_returns_400(self, flask_client, mock_db_connection):
        """POST /api/add with empty or missing new_trackers should return 400."""
        response = flask_client.post("/api/add", data={"new_trackers": ""})
        assert response.status_code == 400

    def test_post_api_add_missing_field_returns_400(self, flask_client, mock_db_connection):
        """POST /api/add without new_trackers field should return 400."""
        response = flask_client.post("/api/add", data={})
        assert response.status_code == 400

    def test_post_api_add_too_long_input_returns_413(self, flask_client, mock_db_connection):
...
class TestApiPercentageEndpoint:
    ...
    def test_get_api_percentage_returns_filtered_trackers(self, flask_client, insert_sample_tracker):
    ...
    def test_get_api_percentage_filters_by_uptime(self, flask_client, insert_sample_tracker):
    ...
    def test_get_api_percentage_zero_returns_all(self, flask_client, insert_sample_tracker):
        """GET /api/0 should return all trackers."""
        response = flask_client.get("/api/0")
        assert response.status_code == 200
        assert b"udp://tracker.example.com:6969/announce" in response.data

    def test_get_api_percentage_100_returns_only_perfect(self, flask_client, insert_sample_tracker):
    ...
    def test_get_api_percentage_invalid_above_100_returns_400(self, flask_client, mock_db_connection):
        """GET /api/<percentage> with percentage > 100 should return 400."""
        response = flask_client.get("/api/101")
        assert response.status_code == 400
        assert response.headers.get("Access-Control-Allow-Origin") == "*"

    def test_get_api_percentage_negative_returns_400(self, flask_client, mock_db_connection):
...
class TestApiStableEndpoint:
    ...
    def test_get_api_stable_returns_95_percent_trackers(self, flask_client, insert_sample_tracker):
    ...
    def test_get_api_stable_excludes_below_95(self, flask_client, mock_db_connection):
...
class TestApiBestEndpoint:
    ...
    def test_get_api_best_redirects_to_stable(self, flask_client, mock_db_connection):
    ...
    def test_get_api_best_follow_redirect(self, flask_client, insert_sample_tracker):
...
class TestApiAllEndpoint:
    ...
    def test_get_api_all_returns_all_trackers(self, flask_client, insert_sample_tracker):
    ...
    def test_get_api_all_includes_low_uptime_trackers(self, flask_client, mock_db_connection):
...
class TestApiLiveEndpoint:
    ...
    def test_get_api_live_returns_online_trackers(self, flask_client, insert_sample_tracker):
    ...
    def test_get_api_live_excludes_offline_trackers(self, flask_client, mock_db_connection):
...
class TestApiUdpEndpoint:
    ...
    def test_get_api_udp_returns_udp_trackers(self, flask_client, insert_sample_tracker):
    ...
    def test_get_api_udp_excludes_http_trackers(self, flask_client, mock_db_connection):
    ...
    def test_get_api_udp_excludes_low_uptime_udp(self, flask_client, mock_db_connection):
...
class TestApiHttpEndpoint:
    ...
    def test_get_api_http_returns_http_trackers(self, flask_client, mock_db_connection):
    ...
    def test_get_api_http_includes_https_trackers(self, flask_client, mock_db_connection):
    ...
    def test_get_api_http_excludes_udp_trackers(self, flask_client, insert_sample_tracker):
...
class TestIpv4Ipv6Filtering:
    ...
    @pytest.fixture
    def insert_ipv4_only_tracker(self, mock_db_connection):
    ...
    @pytest.fixture
    def insert_ipv6_only_tracker(self, mock_db_connection):
    ...
    @pytest.fixture
    def insert_dual_stack_tracker(self, mock_db_connection):
...
class TestSubmittedEndpoint:
    ...
    def test_get_submitted_returns_200(self, flask_client, mock_db_connection):
        """GET /submitted should return 200 OK."""
        response = flask_client.get("/submitted")
        assert response.status_code == 200

    def test_get_submitted_with_data(self, flask_client, mock_db_connection):
...
class TestRawEndpoint:
    ...
    def test_get_raw_returns_200(self, flask_client, mock_db_connection):
        """GET /raw should return 200 OK."""
        response = flask_client.get("/raw")
        assert response.status_code == 200

    def test_get_raw_with_data(self, flask_client, mock_db_connection):
...
class TestStaticPages:
    ...
    def test_get_faq_returns_200(self, flask_client, mock_db_connection):
        """GET /faq should return 200 OK."""
        response = flask_client.get("/faq")
        assert response.status_code == 200

    def test_get_list_returns_200(self, flask_client, mock_db_connection):
        """GET /list should return 200 OK."""
        response = flask_client.get("/list")
        assert response.status_code == 200

    def test_get_api_docs_returns_200(self, flask_client, mock_db_connection):
        """GET /api should return 200 OK."""
        response = flask_client.get("/api")
        assert response.status_code == 200

    def test_get_about_returns_200(self, flask_client, mock_db_connection):
...
class TestAnnounceRequestRejection:
    ...
    def test_reject_request_with_info_hash(self, flask_client, mock_db_connection):
        """Requests with info_hash parameter should be rejected with 403."""
        response = flask_client.get("/?info_hash=abc123")
        assert response.status_code == 403

    def test_reject_api_request_with_info_hash(self, flask_client, mock_db_connection):
...
class TestApiResponseHeaders:
    ...
    def test_api_cors_header(self, flask_client, insert_sample_tracker):
        """API endpoints should include CORS header."""
        response = flask_client.get("/api/95")
        assert response.headers.get("Access-Control-Allow-Origin") == "*"

    def test_api_content_type(self, flask_client, insert_sample_tracker):
        """API endpoints should return plain text."""
        response = flask_client.get("/api/95")
        assert "text/plain" in response.content_type

    def test_api_add_cors_header(self, flask_client, mock_db_connection):
...
class TestEdgeCases:
    ...
    def test_api_percentage_boundary_zero(self, flask_client, insert_sample_tracker):
        """GET /api/0 should be valid and return all trackers."""
        response = flask_client.get("/api/0")
        assert response.status_code == 200

    def test_api_percentage_boundary_100(self, flask_client, mock_db_connection):
        """GET /api/100 should be valid."""
        response = flask_client.get("/api/100")
        assert response.status_code == 200

    def test_empty_database_api_response(self, flask_client, mock_db_connection):
        """API should handle empty database gracefully."""
        response = flask_client.get("/api/95")
        assert response.status_code == 200
        assert response.data == b""

    def test_multiple_trackers_in_response(self, flask_client, mock_db_connection):
    ...
    def test_special_characters_in_tracker_url(self, flask_client, mock_db_connection):
