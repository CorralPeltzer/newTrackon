...
def patched_db(test_db: sqlite3.Connection, monkeypatch: MonkeyPatch) -> sqlite3.Connection:
    ...
    def patched_connect(database: str) -> sqlite3.Connection:
...
@pytest.fixture
def sample_tracker_dict() -> TrackerDataDict:
...
@pytest.fixture
def sample_tracker_obj(sample_tracker_dict: TrackerDataDict) -> Tracker:
...
@pytest.fixture
def inserted_sample_tracker(patched_db: sqlite3.Connection, sample_tracker_dict: TrackerDataDict) -> TrackerDataDict:
    """Insert sample tracker into the test database."""
    _ = patched_db.execute(
...
class TestInsertNewTracker:
    ...
    def test_insert_new_tracker_stores_data_correctly(self, patched_db: sqlite3.Connection, sample_tracker_obj: Tracker) -> None:
        ...
        _ = cursor.execute("SELECT * FROM status WHERE host = ?", (sample_tracker_obj.host,))
    ...
    def test_insert_new_tracker_json_serializes_ips(self, patched_db: sqlite3.Connection, sample_tracker_obj: Tracker) -> None:
        ...
        cursor = patched_db.cursor()
        _ = cursor.execute("SELECT ip FROM status WHERE host = ?", (sample_tracker_obj.host,))
        row = cast(tuple[object, ...] | None, cursor.fetchone())
        assert row is not None

        stored_ips = cast(object, json.loads(cast(str, row[0])))
    ...
    def test_insert_new_tracker_json_serializes_countries(
        ...
        cursor = patched_db.cursor()
        _ = cursor.execute("SELECT country FROM status WHERE host = ?", (sample_tracker_obj.host,))
        row = cast(tuple[object, ...] | None, cursor.fetchone())
        assert row is not None

        stored_countries = cast(object, json.loads(cast(str, row[0])))
    ...
    def test_insert_new_tracker_json_serializes_country_codes(
        ...
        cursor = patched_db.cursor()
        _ = cursor.execute("SELECT country_code FROM status WHERE host = ?", (sample_tracker_obj.host,))
        row = cast(tuple[object, ...] | None, cursor.fetchone())
        assert row is not None

        stored_country_codes = cast(object, json.loads(cast(str, row[0])))
    ...
    def test_insert_new_tracker_json_serializes_networks(
        ...
        cursor = patched_db.cursor()
        _ = cursor.execute("SELECT network FROM status WHERE host = ?", (sample_tracker_obj.host,))
        row = cast(tuple[object, ...] | None, cursor.fetchone())
        assert row is not None

        stored_networks = cast(object, json.loads(cast(str, row[0])))
    ...
    def test_insert_new_tracker_json_serializes_historic(
        ...
        cursor = patched_db.cursor()
        _ = cursor.execute("SELECT historic FROM status WHERE host = ?", (sample_tracker_obj.host,))
        row = cast(tuple[object, ...] | None, cursor.fetchone())
        assert row is not None

        stored_historic = cast(object, json.loads(cast(str, row[0])))
    ...
    def test_insert_new_tracker_with_multiple_ips(self, patched_db: sqlite3.Connection) -> None:
        ...
        cursor = patched_db.cursor()
        _ = cursor.execute("SELECT ip FROM status WHERE host = ?", (tracker.host,))
        row = cast(tuple[object, ...] | None, cursor.fetchone())
        assert row is not None
...
class TestGetAllData:
    ...
    def test_get_all_data_returns_tracker_objects(self) -> None:
    ...
    @pytest.mark.usefixtures("patched_db", "inserted_sample_tracker")
    def test_get_all_data_returns_correct_host(self, sample_tracker_dict: TrackerDataDict) -> None:
    ...
    @pytest.mark.usefixtures("patched_db", "inserted_sample_tracker")
    def test_get_all_data_returns_correct_url(self, sample_tracker_dict: TrackerDataDict) -> None:
    ...
    @pytest.mark.usefixtures("patched_db", "inserted_sample_tracker")
    def test_get_all_data_deserializes_ips(self, sample_tracker_dict: TrackerDataDict) -> None:
    ...
    @pytest.mark.usefixtures("patched_db", "inserted_sample_tracker")
    def test_get_all_data_deserializes_countries(self, sample_tracker_dict: TrackerDataDict) -> None:
    ...
    @pytest.mark.usefixtures("patched_db", "inserted_sample_tracker")
    def test_get_all_data_deserializes_country_codes(self, sample_tracker_dict: TrackerDataDict) -> None:
    ...
    @pytest.mark.usefixtures("patched_db", "inserted_sample_tracker")
    def test_get_all_data_deserializes_networks(self, sample_tracker_dict: TrackerDataDict) -> None:
    ...
    @pytest.mark.usefixtures("patched_db", "inserted_sample_tracker")
    def test_get_all_data_deserializes_historic_as_deque(self, sample_tracker_dict: TrackerDataDict) -> None:
    ...
    def test_get_all_data_returns_empty_list_when_no_trackers(self) -> None:
...
class TestUpdateTracker:
    ...
    @pytest.mark.usefixtures("inserted_sample_tracker")
    def test_update_tracker_updates_url(self, patched_db: sqlite3.Connection, sample_tracker_obj: Tracker) -> None:
        ...
        cursor = patched_db.cursor()
        _ = cursor.execute("SELECT url FROM status WHERE host = ?", (sample_tracker_obj.host,))
        row = cast(tuple[object, ...] | None, cursor.fetchone())
        assert row is not None
    ...
    @pytest.mark.usefixtures("inserted_sample_tracker")
    def test_update_tracker_updates_latency(self, patched_db: sqlite3.Connection, sample_tracker_obj: Tracker) -> None:
        """Verify that update_tracker updates latency."""
        sample_tracker_obj.latency = 200
        db.update_tracker(sample_tracker_obj)

        cursor = patched_db.cursor()
        _ = cursor.execute("SELECT latency FROM status WHERE host = ?", (sample_tracker_obj.host,))
        row = cast(tuple[object, ...] | None, cursor.fetchone())
        assert row is not None
    ...
    @pytest.mark.usefixtures("inserted_sample_tracker")
    def test_update_tracker_updates_status(self, patched_db: sqlite3.Connection, sample_tracker_obj: Tracker) -> None:
        """Verify that update_tracker updates status."""
        sample_tracker_obj.status = 0
        db.update_tracker(sample_tracker_obj)

        cursor = patched_db.cursor()
        _ = cursor.execute("SELECT status FROM status WHERE host = ?", (sample_tracker_obj.host,))
        row = cast(tuple[object, ...] | None, cursor.fetchone())
        assert row is not None
    ...
    @pytest.mark.usefixtures("inserted_sample_tracker")
    def test_update_tracker_updates_uptime(self, patched_db: sqlite3.Connection, sample_tracker_obj: Tracker) -> None:
        """Verify that update_tracker updates uptime."""
        sample_tracker_obj.uptime = 85
        db.update_tracker(sample_tracker_obj)

        cursor = patched_db.cursor()
        _ = cursor.execute("SELECT uptime FROM status WHERE host = ?", (sample_tracker_obj.host,))
        row = cast(tuple[object, ...] | None, cursor.fetchone())
        assert row is not None
    ...
    @pytest.mark.usefixtures("inserted_sample_tracker")
    def test_update_tracker_updates_ips_with_json(self, patched_db: sqlite3.Connection, sample_tracker_obj: Tracker) -> None:
    ...
    @pytest.mark.usefixtures("inserted_sample_tracker")
    def test_update_tracker_updates_historic_with_json(self, patched_db: sqlite3.Connection, sample_tracker_obj: Tracker) -> None:
    ...
    @pytest.mark.usefixtures("inserted_sample_tracker")
    def test_update_tracker_updates_networks_with_json(self, patched_db: sqlite3.Connection, sample_tracker_obj: Tracker) -> None:
    ...
    @pytest.mark.usefixtures("inserted_sample_tracker")
    def test_update_tracker_updates_last_checked(self, patched_db: sqlite3.Connection, sample_tracker_obj: Tracker) -> None:
        """Verify that update_tracker updates last_checked."""
        sample_tracker_obj.last_checked = 1700001000
        db.update_tracker(sample_tracker_obj)

        cursor = patched_db.cursor()
        _ = cursor.execute("SELECT last_checked FROM status WHERE host = ?", (sample_tracker_obj.host,))
        row = cast(tuple[object, ...] | None, cursor.fetchone())
        assert row is not None
    ...
    @pytest.mark.usefixtures("inserted_sample_tracker")
    def test_update_tracker_updates_interval(self, patched_db: sqlite3.Connection, sample_tracker_obj: Tracker) -> None:
        """Verify that update_tracker updates interval."""
        sample_tracker_obj.interval = 3600
        db.update_tracker(sample_tracker_obj)

        cursor = patched_db.cursor()
        _ = cursor.execute("SELECT interval FROM status WHERE host = ?", (sample_tracker_obj.host,))
        row = cast(tuple[object, ...] | None, cursor.fetchone())
        assert row is not None
    ...
    @pytest.mark.usefixtures("inserted_sample_tracker")
    def test_update_tracker_updates_last_downtime(self, patched_db: sqlite3.Connection, sample_tracker_obj: Tracker) -> None:
        """Verify that update_tracker updates last_downtime."""
        sample_tracker_obj.last_downtime = 1700000500
        db.update_tracker(sample_tracker_obj)

        cursor = patched_db.cursor()
        _ = cursor.execute("SELECT last_downtime FROM status WHERE host = ?", (sample_tracker_obj.host,))
        row = cast(tuple[object, ...] | None, cursor.fetchone())
        assert row is not None
    ...
    @pytest.mark.usefixtures("inserted_sample_tracker")
    def test_update_tracker_updates_last_uptime(self, patched_db: sqlite3.Connection, sample_tracker_obj: Tracker) -> None:
        """Verify that update_tracker updates last_uptime."""
        sample_tracker_obj.last_uptime = 1700000999
        db.update_tracker(sample_tracker_obj)

        cursor = patched_db.cursor()
        _ = cursor.execute("SELECT last_uptime FROM status WHERE host = ?", (sample_tracker_obj.host,))
        row = cast(tuple[object, ...] | None, cursor.fetchone())
        assert row is not None
...
class TestDeleteTracker:
    ...
    @pytest.mark.usefixtures("inserted_sample_tracker")
    def test_delete_tracker_removes_tracker(self, patched_db: sqlite3.Connection, sample_tracker_obj: Tracker) -> None:
    ...
    def test_delete_tracker_only_removes_specified_tracker(self, patched_db: sqlite3.Connection) -> None:
        ...
        cursor = patched_db.cursor()
        _ = cursor.execute("SELECT host FROM status")
    ...
    @pytest.mark.usefixtures("patched_db")
    def test_delete_tracker_no_error_for_nonexistent_tracker(self, sample_tracker_obj: Tracker) -> None:
...
class TestGetApiData:
    ...
    def test_get_api_data_http_returns_http_and_https_trackers(self) -> None:
    ...
    @pytest.mark.usefixtures("populated_db")
    def test_get_api_data_udp_returns_udp_trackers(self) -> None:
    ...
    @pytest.mark.usefixtures("populated_db")
    def test_get_api_data_live_returns_all_live_trackers(self) -> None:
    ...
    @pytest.mark.usefixtures("populated_db")
    def test_get_api_data_percentage_returns_trackers_above_threshold(self) -> None:
    ...
    @pytest.mark.usefixtures("populated_db")
    def test_get_api_data_percentage_zero_returns_all_trackers(self) -> None:
    ...
    @pytest.mark.usefixtures("populated_db")
    def test_get_api_data_returns_formatted_string(self) -> None:
    ...
    @pytest.mark.usefixtures("populated_db")
    def test_get_api_data_orders_by_uptime_descending(self) -> None:
    ...
    @pytest.mark.usefixtures("patched_db")
    def test_get_api_data_empty_database(self) -> None:
...
class TestJsonSerializationDeserialization:
    ...
    @pytest.mark.usefixtures("patched_db")
    def test_roundtrip_empty_lists(self) -> None:
    ...
    @pytest.mark.usefixtures("patched_db")
    def test_roundtrip_special_characters_in_strings(self) -> None:
    ...
    @pytest.mark.usefixtures("patched_db")
    def test_roundtrip_unicode_characters(self) -> None:
    ...
    @pytest.mark.usefixtures("patched_db")
    def test_roundtrip_large_historic_deque(self) -> None:
    ...
    @pytest.mark.usefixtures("patched_db")
    def test_roundtrip_multiple_ipv4_and_ipv6(self) -> None:
    ...
    @pytest.mark.usefixtures("patched_db", "inserted_sample_tracker")
    def test_update_preserves_json_structure(self, sample_tracker_obj: Tracker) -> None:
...
class TestDatabaseCreation:
    ...
    def test_create_db_creates_status_table(self, tmp_path: Path, monkeypatch: MonkeyPatch) -> None:
        ...
        cursor = conn.cursor()
        _ = cursor.execute("PRAGMA table_info(status)")
        columns = {row[1]: row[2] for row in cast(list[tuple[object, ...]], cursor.fetchall())}
        sqlite3.Connection.close(conn)
    ...
    def test_create_db_host_is_primary_key(self, tmp_path: Path, monkeypatch: MonkeyPatch) -> None:
        ...
        pk_columns = [row[1] for row in cast(list[tuple[object, ...]], cursor.fetchall()) if row[5] != 0]
        sqlite3.Connection.close(conn)
